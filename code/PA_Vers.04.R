...
checkpoint_times_clean <- checkpoint_times_clean %>%
  mutate(
    Wochentag_num = wday(TrackTime, week_start = 1),
    Wochentag     = factor(
      Wochentag_num,
      levels = 1:7,
...
Wochentag_stats <- checkpoint_times_clean %>%
  filter(!is.na(duration_to_next)) %>%
  group_by(Wochentag) %>%
  summarise(
    n                = n(),
...
ggplot(checkpoint_times_clean %>% filter(!is.na(duration_to_next)),
       aes(x = Wochentag, y = duration_to_next, fill = Wochentag)) +
  geom_boxplot(outlier.alpha = 0.2) +
  theme_minimal() +
  labs(
...
ggplot(Wochentag_stats,
       aes(x = Wochentag, y = median_duration, group = 1)) +
...
cp_Wochentag <- kritische_daten %>%
  group_by(CheckpointID, Wochentag) %>%
  summarise(
    median_duration = median(duration_to_next),
    n               = n(),
    .groups         = "drop"
...
ggplot(cp_Wochentag,
       aes(x = Wochentag, y = median_duration,
           color = factor(CheckpointID),
           group = CheckpointID)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
...
prozesse_ankunft <- prozesse_clean %>%
  mutate(
    ...
    Wochentag_num = wday(start_time, week_start = 1),
    Wochentag     = factor(
      Wochentag_num,
      levels = 1:7,
      labels = c("Mo","Di","Mi","Do","Fr","Sa","So")
...
ankunft_wochentag <- prozesse_ankunft %>% count(Wochentag)
...
ankunft_heat <- prozesse_ankunft %>%
  count(Wochentag, schicht) %>%
  mutate(schicht = factor(schicht,
                          levels = c("Früh", "Spät", "Nacht")))

ggplot(ankunft_heat,
       aes(x = Wochentag, y = schicht, fill = n)) +
  geom_tile() +
  scale_fill_viridis_c() +
  theme_minimal() +
  labs(
...
prozesse_abgang <- prozesse_clean %>%
  mutate(
    ...
    Wochentag_num_end = wday(end_time, week_start = 1),
    Wochentag_end     = factor(
      Wochentag_num_end,
      levels = 1:7,
      labels = c("Mo","Di","Mi","Do","Fr","Sa","So")
    ),
...
abgang_wochentag <- prozesse_abgang %>% count(Wochentag_end)

ggplot(abgang_wochentag,
       aes(x = Wochentag_end, y = n)) +
...
# Heatmap Abgänge
abgang_heat <- prozesse_abgang %>% count(Wochentag_end, schicht_end)

ggplot(abgang_heat,
       aes(x = Wochentag_end, y = schicht_end, fill = n)) +
  geom_tile() +
  scale_fill_viridis_c() +
  theme_minimal() +
  labs(
...
model_Wochentag <- lm(duration_h ~ Wochentag,    data = prozesse_ankunft)

model_full <- lm(duration_h ~ start_hour + schicht + Wochentag,
                 data = prozesse_ankunft)

summary(model_hour)
summary(model_schicht)
summary(model_Wochentag)
summary(model_full)
...
heat_week <- prozesse_ankunft %>%
  count(Wochentag, name = "starts") %>%
  left_join(
    prozesse_abgang %>% count(Wochentag_end, name = "ends"),
    by = c("Wochentag" = "Wochentag_end")
  ) %>%
  mutate(diff = starts - ends)

ggplot(heat_week,
       aes(x = Wochentag, y = 1, fill = diff)) +
  geom_tile() +
  scale_fill_viridis_c() +
  theme_minimal() +
  labs(
...
starts_Wochentag <- prozesse_ankunft %>% 
  count(Wochentag)

# Anzahl Prozessenden pro Wochentag
ends_Wochentag <- prozesse_abgang %>% 
  count(Wochentag_end) %>% 
  rename(Wochentag = Wochentag_end)

# Median-Prozessdauer pro Wochentag
dauer_Wochentag <- prozesse_ankunft %>%
  group_by(Wochentag) %>%
  summarise(
...
plot_Wochentag <- starts_Wochentag %>%
  rename(starts = n) %>%
  left_join(ends_Wochentag %>% rename(ends = n), by = "Wochentag") %>%
  left_join(dauer_Wochentag, by = "Wochentag") %>%
  mutate(
...
# Plot
ggplot(plot_Wochentag, aes(x = Wochentag)) +
